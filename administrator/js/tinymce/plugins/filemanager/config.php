...
//   |    |   |    |-   |-      |- thumbs <- folder of thumbs [must have the write permission]
	...
	if($_SESSION['s_script']=='create_thread.php')
	{
		$script_images = 'thread';
	} else if($_SESSION['s_script']=='create_news.php')
	{
		$script_images = 'news';
	} else if($_SESSION['s_script']=='create_record.php')
	{
		$script_images = 'record';
	} else 
	{
		exit;
	}
$script_images = "editor";
$base_url="http://".$_SERVER['HTTP_HOST']."/"; //url base of site if you want only relative url leave empty
$upload_dir = '/images/data/'; // path from base_url to upload base dir	

$upload_dir = '/ivn.vn/customer/web/data/images/'.$script_images."/"; // path from base_url to upload base dir	
$current_path = '../../../../../data/images/'.$script_images."/"; // relative path from filemanager folder to upload files folder
