	...
	<div class="span12">
		...
		foreach ($files as $file) {
			...
			if ($file != '.' && $file != '..' && !is_dir($root . $cur_dir . $file)) {
				...
				$check_create_thumbs  = dirname( __FILE__ ). DIRECTORY_SEPARATOR.$current_path.$subfolder.DIRECTORY_SEPARATOR.$subdir.$file;	
				
				$check_create_thumbs = str_replace("//","/",$check_create_thumbs);
				if(!file_exists($thumb_path)) 
				{	
					$arr_check =  getimagesize($check_create_thumbs);
					if(strpos($arr_check['mime'],strtolower($ext))!==false)
					{
						@create_img_gd($check_create_thumbs, $thumb_path, 122, 91);
					} else 
					{
						continue;
					}
